...
function touchMoved() {
  ...
  var maxDist = 9999;
  for (var i=0; i<myParticles.length; i++) {
    var dx = touchX - myParticles[i].px;
    var dy = touchY - myParticles[i].py;
    var dh = sqrt(dx*dx + dy*dy);
    if (dh < maxDist) {
      maxDist = dh;
      whichParticleIsGrabbed = i;
  ...
  if(touchY<ledgeHeight && touchX<width/2+41 && touchX>width/2-260){
    ledgeActivated = true;
  }else{
    ledgeActivated = false;
  }
  print("touch recieved at "+touchX+" , "+touchY);
  return false;
} 
...
function draw() {
  ...
  if ((mouseIsPressed || touchIsDown) && (whichParticleIsGrabbed > -1)) {
    // If the user is grabbing a particle, peg it to the mouse.
    myParticles[whichParticleIsGrabbed].px = touchX;
    myParticles[whichParticleIsGrabbed].py = touchY;
  }
